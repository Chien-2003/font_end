  ...
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <Button
          variant="ghost"
          className="relative text-white hover:bg-white/20 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full transform transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-xl px-4 py-2 group"
        >
      ...
      <AlertDialogContent className="max-w-2xl border-0 shadow-2xl bg-gradient-to-br from-primary to-primary/40 backdrop-blur-3xl h-[90%] overflow-y-auto [&::-webkit-scrollbar]:hidden scrollbar-hide">
        <AlertDialogHeader className="text-center space-y-3">
          ...
          <AlertDialogTitle className="text-2xl font-bold text-background dark:text-white">
            Bộ lọc sản phẩm
          </AlertDialogTitle>
          <AlertDialogDescription className="text-background dark:text-white text-base">
                ...
                <div key={option.value} className="relative">
                  <div
                    className={`flex items-center p-4 rounded-xl border-2 cursor-pointer transition-all duration-300 ${
                      isSelected
                        ? 'border-blue-500 bg-blue-100/50 shadow-md transform scale-[1.02]'
                      ...
                      <div className="flex-1">
                        <Label
                          ...
                          className={`
                            ...
                            ${isSelected ? 'text-blue-700' : 'text-gray dark:text-white'}
                          `}
                        >
                          {option.label}
                        </Label>
                        <p
                          className={`text-sm mt-1 transition-colors ${isSelected ? 'text-blue-600' : 'text-gray-800'}`}
                        >
                          {option.description}
                        </p>
                      </div>
        ...
        <AlertDialogFooter className="gap-3 pt-6 border-t border-gray-200/50">
          <AlertDialogCancel className="flex-1 rounded-xl transition-all duration-200 py-3 h-12">
            Đóng
          </AlertDialogCancel>
          <AlertDialogAction
            className="flex-1 rounded-xl bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 py-3 h-12"
