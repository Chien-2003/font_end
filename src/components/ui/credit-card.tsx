...
const validateCreditCard = (
  ...
  if (!value.cardholderName?.trim()) {
    ...
    errors.cardholderName = 'Tên phải có ít nhất 2 ký tự';
  }
  ...
  if (!cleanCardNumber) {
    ...
    errors.cardNumber = 'Độ dài số thẻ không hợp lệ';
  } else if (!/^\d+$/.test(cleanCardNumber)) {
    errors.cardNumber = 'Số thẻ chỉ được chứa chữ số';
  }
  if (!value.expiryMonth?.trim()) {
    errors.expiryMonth = 'Tháng hết hạn là bắt buộc';
  }
  if (!value.expiryYear?.trim()) {
    errors.expiryYear = 'Năm hết hạn là bắt buộc';
  ...
  if (!value.cvv?.trim()) {
    errors.cvv = `${cvvLabel} là bắt buộc`;
  } else if (value.cvv.length !== expectedCvvLength) {
    errors.cvv = `${cvvLabel} phải là ${expectedCvvLength} chữ số`;
  } else if (!/^\d+$/.test(value.cvv)) {
    errors.cvv = `${cvvLabel} chỉ được chứa các chữ số`;
  }
...
function CreditCard({
  ...
  useEffect(() => {
    if (ref && 'current' in ref) {
      ref.current = {
        ...
        isValid: () =>
          Object.keys(validateCreditCard(currentValue, cvvLabel))
            .length === 0,
        focus: handleFocus,
        reset: handleReset,
        getErrors,
  ...
  return (
    <div
      ref={containerRef}
      className={cn('w-full max-w-md py-2', className)}
    >
              ...
              <div className="flex justify-between items-end">
                <div className="">
                  ...
                  <div className="font-bold text-sm">
                    {currentValue.cardholderName || 'TÊN CỦA BẠN'}
                  </div>
                </div>
